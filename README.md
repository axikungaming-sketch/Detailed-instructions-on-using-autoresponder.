...
| 6 | **`_pages(p, uid)`** | 229–1042 | **Khối lớn nhất** — "kho dữ liệu" toàn bộ nội dung help: mỗi key (`"farm"`, `"economy"`, `"casino"`...) là 1 dict `{label, color, title, desc, fields}`, hỗ trợ đa ngôn ngữ. Đây là nơi **duy nhất** cần sửa khi muốn đổi nội dung 1 trang help. |
| 7 | `_build(page_key, prefix, uid)` | 1043–1067 | Lấy 1 entry từ `_pages()` và dựng thành `discord.Embed` thật. |
| 8 | `RoastWarningView`, `CasinoSelect`/`CasinoHelpView` | 1068–1273 | View cảnh báo nội dung 18+ (roast) và view phụ điều hướng riêng cho danh mục casino (nhiều game con). |
| 9 | `PaginatedCommandsView` | 1274–1337 | View phân trang tái sử dụng cho danh mục có danh sách lệnh dài (vd. Thám Hiểm Rừng — 2 trang). |
| 10 | `AutoresponderHelpView` | 1338–1382 | View phân trang **9 trang tĩnh** (`autoresponder`…`autoresponder9` trong `_pages()`) mô tả cú pháp `!ar` ngay trong menu `!help`, không cần rời sang lệnh `!ar`. |
| 11 | `Part4View` / `Part4Select` | 1384–1486 | UI riêng cho Phần 4 (Interactions/Ratings/Anime/Manga) — 4 select-menu con trên cùng 1 view. |
| 12 | `MainPartSelect` → `PartCategorySelect` → `PartCategoryView` → `HelpView` | 1487–1687 | Chuỗi điều hướng chính: **Trang chủ → chọn Phần → chọn danh mục trong Phần → xem embed**, nút "🔙" quay lại đúng cấp trước. |
| 13 | `EconomyCog` | 1690–1775 | Cog thật — nơi đăng ký các lệnh Discord (`!help`, `!sync`, `/help`, `!setprefix`) và listener `on_message` cho việc bật/tắt music. |
...
AutoresponderEngine  (class chính — parse + render 1 reply)
   ...
   ├─ render_text()           → thay placeholder + chạy hàm chuỗi/toán học thuần
...
```
{loop:N}  hoặc  {loop:A-Z}
{start} ... nội dung lặp (dùng [loop] để in số thứ tự) ... {stop}
```
...
#### K. 50 khối chuỗi & số học thuần (Nhóm A+B + Nhóm I)

**Chuỗi (30):** `{uppercase:} {lowercase:} {titlecase:} {capitalize:} {reverse:} {length:} {trim:} {repeat:x|n} {replace:text|old|new} {contains:text|sub} {startswith:} {endswith:} {substring:text|start|end} {concat:a|b|c} {join:sep|a|b|c} {wordcount:} {charat:text|idx} {padleft:text|n|ch} {padright:} {truncate:text|n} {urlencode:} {slugify:} {removespaces:} {countchar:text|ch} {shuffletext:} {leet:} {mocktext:} {emojify:} {rot13:} {maskcenter:}`

**Toán học (15):** `{math:1+2*3} {roundnum:x|digits} {floornum:} {ceilnum:} {absnum:} {minnum:a|b} {maxnum:a|b} {sumnum:a|b|c} {avgnum:a|b|c} {percent:phần|tổng} {modnum:a|b} {pownum:base|exp} {sqrtnum:} {clamp:v|min|max} {randomfloat:min|max}`
...
`PART_MAP` trong `economy.py` có `7: ["welcome", "level", "autoresponder"]`. Khi người dùng chọn danh mục **"autoresponder"** trong `!help`, `PartCategorySelect.callback` mở `AutoresponderHelpView` — view này đọc nội dung từ **chính `_pages()` của economy.py** (9 trang: `autoresponder` → `autoresponder9`), **không** gọi hàm `_ar_help_embed()` bên trong `autoresponder.py`.

➡️ Nghĩa là có **2 nguồn nội dung help tách biệt cho cùng 1 tính năng**:

| Nguồn | Nơi | Số trang |
|---|---|---|
| `!help` → mục Autoresponder | `_pages()` trong `economy.py` | 9 trang tĩnh |
| `!ar` (gõ trực tiếp) | `_ar_help_embed()` trong `autoresponder.py` | 5 trang |

**Hệ quả khi bạn thêm block mới vào `autoresponder.py`:** cần cập nhật **cả 2 nơi** nếu muốn nội dung khớp nhau, vì chúng không tự đồng bộ. Cách gọn hơn nếu muốn tránh trùng lặp: sửa `AutoresponderHelpView.build_embed()` trong `economy.py` để gọi thẳng `_ar_help_embed()` (import từ `cogs.autoresponder`) thay vì dùng `_pages()` riêng — khi đó chỉ cần sửa 1 chỗ.
